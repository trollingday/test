   	...
   	<div id="container">
   		<div id="contents">
   			...
   			<div id="section2">
				...
				<!-- 우측내용 시작 -->
				<div id="right">
					<h2 align="center">환불 요청 목록</h2>
					<form action="${path}/refundKeyword.sc?${_csrf.parameterName}=${_csrf.token}" method="post">
						<div>
					...
					<div class="table_div">
   						<table style="width:1000px" align="center">
   							<tr>
								...
								<th style="width:5%" colspan=2>환불</th>																							   								
   							</tr>
   									
   							<!-- 게시글이 있으면 -->
   							<c:if test="${refundBox!=null}">
	   							<c:forEach var="row" items="${refundBox}">
		   							<tr>
		   								...
		   								<td ><input type="button" value="승인" onclick="window.location='${path}/refundPermit.od?${_csrf.parameterName}=${_csrf.token}&orderNo=${row.getOrderNo()}&orderState=${row.getOrderStatus()}'"></td>   
		   								<td ><input type="button" value="거절" onclick="window.location='${path}/refundReject.od?${_csrf.parameterName}=${_csrf.token}&orderNo=${row.getOrderNo()}&orderState=${row.getOrderStatus()}'"></td>   		   						   								  								   								
		   							</tr>
	     						</c:forEach>
     						</c:if>
